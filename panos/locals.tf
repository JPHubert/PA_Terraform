...
locals {
    pa_count        =   "2"
    subnet_mgmt     =   "${cidrsubnet("${var.tran_vnet_address}", 5, 2)}"
    subnet_untrust  =   "${cidrsubnet("${var.tran_vnet_address}", 5, 0)}"
    subnet_trust    =   "${cidrsubnet("${var.tran_vnet_address}", 5, 1)}"
#    subnet_shared   =   "${cidrsubnet("${var.tran_vnet_address}", 8, 3)}"
#    subnet_egress   =   "${cidrsubnet("${var.tran_vnet_address}", 8, 4)}"
    subnet_dmz      =   "${cidrsubnet("${var.tran_vnet_address}", 3, 2)}"
